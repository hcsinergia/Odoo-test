...
class BM_Official(models.Model):
    ...
    segmentation = fields.Selection([
        ('ecovis', 'ECOVIS'),
        ('ecovis_plus', 'ECOVIS PLUS'),
        ('ecovis_elite', 'ECOVIS ELITE')], size=1, string="Recomendación Segmentación")
    ...
    def notify_to_channel_users(self, _channel, _subject, _message):
        try:
            ...
            channel_id = self.env.ref('hcs_bm_ecovis.{}'.format(_channel))
            if channel_id:
    ...
    def notify_to_company_users(self, _company_id, _subject, _message):
        try:
            ...
            # Obtengo los contactos de cada usuario dentro de la compañia principal y La Empresa
            users_partner_ids = []
